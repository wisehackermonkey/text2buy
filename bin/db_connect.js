...
var tester={
	...
	"post_items":function(data,callback){
		...
		var query_post=`INSERT INTO posts(uaid,title,description,price,city,expireson,created,modified) values(15,"${data.title}","${description}","${data.price}","${data.city}",'datetime()','datetime()','datetime()')`;
		db.run(query_post,function(err){
 			console.log(err);
 			var response=[JSON.parse("{\"msg\":\"successfully added\"}")];
 			callback(response);
 		});
	}
};
